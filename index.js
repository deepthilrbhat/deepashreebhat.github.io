...
import { URLs } from './user-data/urls.js';
  ...
  function populateBio(items, id) {
   ...
   /**
   * Populates leadership philosophy to the HTML page.
   *
   * @function
   *
   * @param {Array} items - An array of objects that contain bio information.
  ...
  function populateLeader(items, id) {
    const leaderTag = document.getElementById(id);
    items.forEach((leaderItem) => {
      const p = getElement("p", null);
      p.innerHTML = leaderItem;
      leaderTag.append(p);
    });
  }
  ...
  populateBio(bio, "bio");
  populateLeader(leadershipPhilosophy, "leader");

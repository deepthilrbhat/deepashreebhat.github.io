...
import { URLs } from '/user-data/urls.js';
  ...
  const { teamBuilding, dataDriven, dataManagement, projectManagement } =
    whatido;
  /**
  ...
  /**
   * Populates whatido to the HTML page.
   *
   * @function
   *
   * @param {Array} items - An array of objects that contain project information.
  ...
  function populateWhatido(items, id) {

    // Select the `<ul>` element where the bullet points will be added
    const bulletList = document.getElementById(id);

    items.forEach((pointItem) => {
      const listItem = document.createElement('li'); // Create a new list item
      listItem.textContent = pointItem; // Set the text content

      bulletList.appendChild(listItem); // Append the item to the list
    });

     
  }

  populateWhatido(teamBuilding, "teamBuilding");
  populateWhatido(dataDriven, "dataDriven");
  populateWhatido(dataManagement, "dataManagement");
  populateWhatido(projectManagement, "projectManagement");
